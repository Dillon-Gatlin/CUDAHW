...
//Since we are using constant memory we only need the CPU copy of the spheres
sphereStruct *SpheresCPU;

//----------------------ADDED constant memory for spheres----------------------
__constant__ sphereStruct SphereConstant[NUMSPHERES];
...
// -------------------summary of changes to makeSphersBitMap kernel----------------------
// 1: Changed the kernel to use SphereConstant instead of SpheresGPU
// 2: Chnged the parameters to only take in the pixels array
//--------------------------------------------------------------------------------------
...
__global__ void makeSphersBitMap(float *pixels)
{
	...
	{
		hitValue = hit(pixelx, pixely, &dimingValue, SphereConstant[i]);
		...
		if(maxHit < hitValue)
		{
			...
			pixelr = SphereConstant[i].r * dimingValue; 	
			pixelg = SphereConstant[i].g * dimingValue;	
			pixelb = SphereConstant[i].b * dimingValue; 	
...
//-------------Summary of changes to makeBitMap function----------------------
// 1: Removed cudaMalloc and cudaMemcpy for SpheresGPU
// 2: Added cudaMemcpyToSymbol to copy SpheresCPU to SphereConstant
// 3: Changed makeSphersBitMap kernel to use SphereConstant instead of S
// 4: Added CUDA events to time the code
// 4.1: cude event measures how long gpue operatoin take, record starts the time and stops.
// 4.2: synchronize makes sure the event is done before we move on. elapsedTime calculates the time
// 4.3: print out the time it took to do the GPU operations, destroy the events to free memory
// Note I started the timer beefore the cudaMemcpyToSymbol because that is part of the GPU work
// and I stopped the timer after the cudaMemcpyAsync because that is also part of the GPU work.
// I was unsure if we were to time just the kernel or all the GPU work.
//-----------------------------------------------------------------------------
...
{	
	...
	//----------------------Changed to copy to constant memory----------------------
	...
	//----------------------Changed to copy to constant memory----------------------
	//using cudaMemcpyToSymbol to copy to constant memory.
	//cudaMemcpy(SphereConstant, SpheresCPU, NUMSPHERES*sizeof(sphereStruct), cudaMemcpyHostToDevice);
	cudaMemcpyToSymbol(SphereConstant, SpheresCPU, NUMSPHERES * sizeof(sphereStruct));
	cudaErrorCheck(__FILE__, __LINE__);
	...
	printf("Code execution time: %.3f ms aka %.3f s\n", elapsedTime, elapsedTime / 1000.0f);
...
//-------------Summary of changes to setup function----------------------
// 1: Removed cudaMalloc for SpheresGPU since we are using constant memory
// ---------------------------------------------------------------------
void setup()
{
