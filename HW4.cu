...
-------hard coded the block size and grid size as requested
------- set the block size to 512 and grid size to 128 and saw a better performance on the gpu.
------- but it was still spiractic with output times going from 486 up to 758, using 256 and 64 it was also spiractic jumping from 551-967 (mainly around 600-700)
...
-----implemented the new compuation in CPU and GPU Addition functions.
...
------the code was already in a for looop so just implemented the pragma unroll.
------the unroll did not make any difference to speed in fact it was about the same time.
*/
...
void setUpDevices()
{
	...
	GridSize.x = 64;
	//(N - 1)/BlockSize.x + 1; // This gives us the correct number of blocks.
	GridSize.y = 1;
	GridSize.z = 1;
}
...
{
	#pragma unroll 24 //this will unroll the loop 4 times which should help with speed.
	//testing 4,8,12,24 i did not see any great speed up, it measured around the same time.
