...
// this function will allow us to check any errors that occur. since errors occur GPU we need this
//to find specfics and where it occurs at.
void cudaErrorCheck(const char *file, int line)
{
	cudaError_t  error;
...
void setUpDevices()
{
	BlockSize.x = 256; //Change the block size
	BlockSize.y = 1;
	BlockSize.z = 1;
	
	GridSize.x = (N-1)/BlockSize.x+1; //formula used to give correct number of blocks (went over in class)
	GridSize.y = 1;
	GridSize.z = 1;
}
...
{
	//block dim.x*blockIdx.x shows how many threads came before 
	//1 (block) * 256(threads) means we start at 256 (for block 2) since 0-255 have already been completed.
	//allow us to correctly give the ID for each block
