...
//hierachy levels are Grid>block>Threads
void setUpDevices()
{
	//BlockSize.x = N; //Base code
	BlockSize.x = 1024; //max number of threads in cuda is 1024 so n=1500 wouldnt work
	BlockSize.y = 1;
	BlockSize.z = 1;
...
//Since our N is greater than max thread count we need to use a loop since we are restricted to one block
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	//unique ID for the block
	int id = threadIdx.x;

	//each thread will handle one element then if the 1024 cap is reached it will handle a second 
	//this continues until the N length is reached. 
	//Example[i=thread id] (i = 0 = 1 and 1024) (i=1 = 1 and 1025) (i=475 = 475 and 1499)
	//the remaing threads 476 to 1023 will only do 1 element

	//i<n decides how much work is left
	//i += blockDim.x spreads the work. 
	// after first element they jump ahead by blockDim.x size and grabs a new element to work with if i <n.
...
int main()
{
	...
	// The object reciving the copy will be put first, and the orginal second	
	cudaMemcpyAsync(A_GPU, A_CPU, N*sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpyAsync(B_GPU, B_CPU, N*sizeof(float), cudaMemcpyHostToDevice);
	
	//<<<must be put in order for GPU Function calls
	addVectorsGPU<<<GridSize,BlockSize>>>(A_GPU, B_GPU ,C_GPU, N);
	
	// Copy Memory from GPU to CPU	
	//Copy the GPU's work back into the CPU
	cudaMemcpyAsync(C_CPU, C_GPU, N*sizeof(float), cudaMemcpyDeviceToHost);
	
	// Making sure the GPU and CPU wait until each other are at the same place.
	//Without this errors can happen frequently as GPU may not be done with its work.
