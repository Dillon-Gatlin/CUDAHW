...
void dotProductCPU(float *a, float *b, float *C_CPU, int n)
{
	...
	printf("\n The dot product on the CPU is %f", C_CPU[0]);
}

//----------------------------------Summary of dortProductGPU kernel----------------------------------
...
{
	//1
	__shared__ float storage[1000]; // double for accumulation

	//2
    int tid = threadIdx.x;

    //3 = Step 1: load valid elements
    if (tid < n)
	{
		storage[tid] = a[tid] * b[tid];
	}
	else
	{
		storage[tid] = 0.0;
    ...
    //4 = Step 2: folding portion
    while (n > 1)
    {
		//this takes car of the odd protion by adding and using integer division to achieve correct folding
        int half = (n + 1) / 2;
        ...
        __syncthreads();
        n = half; // halve number of active elements
    }

    //6 = Step 3: thread 0 writes final result
    if (tid == 0){
        C_GPU[0] = storage[0];
		//added print statment for double checking and bug fixing
		printf("\n The dot product on the GPU is %f", C_GPU[0]);
	}
}
...
int main()
{
	...
	// Copy only the final dot product from GPU to CPU
