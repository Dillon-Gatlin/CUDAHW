...
// nvcc V_nBodySpeedChallenge2.cu -o temp -lglut -lm -lGLU -lGL   
//-03 enables all compiler optimizations 
...
dim3 GridSize;
//
float GMij, HMij, mass,mass_recip;
...
{	
	...
	printf("\n The compute time was %ld microseconds. aka %f seconds\n\n", computeTime, computeTime/1e6);
}
...
void setup()
{
	...
	// Compute constants used in Force function
    //since all the masses are the same we can precompute G*m[i]*m[j] and H*m[i]*m[j]
    GMij = G * mass * mass;  // G * m[i] * m[j]
    HMij = H * mass * mass;  // H * m[i] * m[j]
    mass_recip = 1.0f / mass; // 1/m[i]
	...
	cudaErrorCheck(__FILE__, __LINE__);
	//speeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed
    //cudaDeviceSetCacheConfig(cudaFuncCachePreferL1);           // Prefer L1 cache over shared memory
    //cudaDeviceSetSharedMemConfig(cudaSharedMemBankSizeFourByte); // Optimize shared memory bank size
	printf("\n To start timing go to the nBody window and type s.\n");
	printf("\n To quit type q in the nBody window.\n");
}

__global__ void getForces(float3 *__restrict__ p, float3 *__restrict__ v, float3 *__restrict__ f, float *__restrict__ m, float GMij, float HMij, int n)
{
	...
	if(i < n)
	{
		...
		for(int j = 0; j < n; j++)
		{
			if(i != j)
			{
				...
				d2 = dx*dx + dy*dy + dz*dz + 1e-6f;
				//d  = sqrt(d2);
                //-------------------------------------speed up-----
                d = rsqrtf(d2); 
                rsqD2 = d * d;  // 1/d^2
                rsqD4 = rsqD2 * rsqD2; // 1/d^4

				
				force_mag  = (GMij)*(rsqD2) - (HMij)*(rsqD4);
				f[i].x += force_mag*dx * d; //speeeeeeeeeeed
...
__global__ void moveBodies(float3 *__restrict__ p, float3 *__restrict__ v, float3 *__restrict__ f, float mass_recip, float damp, float dt, float t, int n)
{	
	int i = threadIdx.x + blockDim.x*blockIdx.x;
	...
	if(i < n)
	{
		...
		if(t == 0.0f) dt*=0.5f; // 
		{
			v[i].x += ((f[i].x-damp*v[i].x)*mass_recip)*dt;
			v[i].y += ((f[i].y-damp*v[i].y)*mass_recip)*dt;
			v[i].z += ((f[i].z-damp*v[i].z)*mass_recip)*dt;
		}
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
		getForces<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, MGPU, GMij, HMij, N);
		cudaErrorCheck(__FILE__, __LINE__);
		moveBodies<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, mass_recip, Damp, dt, t, N);
		cudaErrorCheck(__FILE__, __LINE__);
