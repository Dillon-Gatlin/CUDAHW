...
int main()
{
	...
	cudaErrorCheck(__FILE__, __LINE__);
	/*--------------------------------------------------------------------
    We use two streams to overlap the memory transfer and kernal computation.
    The idea is to use one stream to handle one chunk of data while the other
    stream handles another chunk of data. This way the GPU is always busy either
    computing or transferring data. The reason two is bettr is because while it allows
    for overlap, it does not use too much memory on the GPU.

    So Stream0 will handle the first chunk of data while Stream1 handles the second chunk.
    When Stream0 is done with its chunk, it will move to the third chunk  and so on and so forth.
   
    Note: to make this work we use+ i for stream 0 and + i + DATA_CHUNKS for stream 1.
    This way each stream is working on its own chunk of data.
    Also note that the loop increments by DATA_CHUNKS*2 because each iteration
    processes two chunks of data (one for each stream).
    --------------------------------------------------------------------*/
	for(int i = 0; i < ENTIRE_DATA_SET; i += DATA_CHUNKS*2)
	{
	//------------------Stream 0------------------
	cudaMemcpyAsync(A0_GPU, A_CPU + i, DATA_CHUNKS * sizeof(float),
	                cudaMemcpyHostToDevice, Stream0);
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMemcpyAsync(B0_GPU, B_CPU + i, DATA_CHUNKS * sizeof(float),
	                cudaMemcpyHostToDevice, Stream0);
	cudaErrorCheck(__FILE__, __LINE__);
	...
	cudaMemcpyAsync(C_CPU + i, C0_GPU, DATA_CHUNKS * sizeof(float),
	                cudaMemcpyDeviceToHost, Stream0);
	cudaErrorCheck(__FILE__, __LINE__);

	//------------------------Stream 1------------------
	cudaMemcpyAsync(A1_GPU, A_CPU + i + DATA_CHUNKS, DATA_CHUNKS * sizeof(float),
	                cudaMemcpyHostToDevice, Stream1);
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMemcpyAsync(B1_GPU, B_CPU + i + DATA_CHUNKS, DATA_CHUNKS * sizeof(float),
	                cudaMemcpyHostToDevice, Stream1);
	cudaErrorCheck(__FILE__, __LINE__);
	...
	cudaMemcpyAsync(C_CPU + i + DATA_CHUNKS, C1_GPU, DATA_CHUNKS * sizeof(float),
