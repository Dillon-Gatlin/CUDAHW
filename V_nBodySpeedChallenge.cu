...
__global__ void getForces(float3 *p, float3 *v, float3 *f, float *m, float GMij, float HMij, int n)
{
...
__global__ void moveBodies(float3 *p, float3 *v, float3 *f, float mass_recip, float damp, float dt, float t, int n)
{	
	int i = threadIdx.x + blockDim.x*blockIdx.x;
