...
/*---------------------------Changes
1-used cudaMemPrefetchAsync to prefetch position and velocity data to CPU before drawing
...
void drawPicture()
{
	...
	//1 Prefetch everything to CPU
	cudaMemPrefetchAsync(P, N * sizeof(float3), cudaCpuDeviceId);
	cudaMemPrefetchAsync(V, N * sizeof(float3), cudaCpuDeviceId);
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
       for (int d = 0; d < NumberOfGpus; ++d)
        {
            cudaSetDevice(d);                 // Set GPU d
            int offset = d * NPerGPU;         // start of chunk
            int chunkSize = (d == NumberOfGpus - 1) ? (N - offset) : NPerGPU; // last GPU may have fewer

            // Prefetch only the chunk for this GPU
            cudaMemPrefetchAsync(P + offset, chunkSize * sizeof(float3), d);
            cudaMemPrefetchAsync(V + offset, chunkSize * sizeof(float3), d);
            cudaMemPrefetchAsync(F + offset, chunkSize * sizeof(float3), d);
            cudaMemPrefetchAsync(M + offset, chunkSize * sizeof(float), d);
            cudaDeviceSynchronize(); // Ensure prefetch completes

			getForces<<<GridSize, BlockSize>>>(P, V, F, M, G, H, NPerGPU, N, d);
            cudaErrorCheck(__FILE__, __LINE__);
            moveBodies<<<GridSize, BlockSize>>>(P, V, F, M, Damp, dt, t, NPerGPU, N, d);
            cudaErrorCheck(__FILE__, __LINE__);
        }
        cudaDeviceSynchronize();
        cudaErrorCheck(__FILE__, __LINE__);
