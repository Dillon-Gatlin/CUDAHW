...
/*----------------------------------------Setup changes---------------------------------------
1- used threadCount variable to set number of threads per block.
2- formula to calculate number of blocks needed based on N amount of bodies
...
void setup()
{
    ...
    BlockSize.x = threadCount;
	BlockSize.y = 1;
	BlockSize.z = 1;
	//-------------------2.--------------------
	GridSize.x = (N + threadCount - 1) / threadCount;
	GridSize.y = 1;
	GridSize.z = 1;
...
/*----------------------------------------Changes made
1- change I to use global index
2- added bounds check to make sure we do not go out of bounds
